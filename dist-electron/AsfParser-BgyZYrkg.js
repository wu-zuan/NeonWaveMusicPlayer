import { u as H, h as P, s as V, e as $, f as l, j as o, k as E, m as W, l as A, S as h, A as _, B as J, c as q, T as z } from "./main-5xIj-nOV.js";
class e {
  ...
  static decode(t, a = 0) {
    ...
    return `${r.getUint32(0, !0).toString(16)}-${r.getUint16(4, !0).toString(16)}-${r.getUint16(6, !0).toString(16)}-${r.getUint16(8).toString(16)}-${H(t.subarray(a + 10, a + 16))}`.toUpperCase();
  }
  /**
   * Decode stream type
   ...
   */
  static encode(t) {
    const a = new Uint8Array(16), r = new DataView(a.buffer);
    return r.setUint32(0, Number.parseInt(t.substring(0, 8), 16), !0), r.setUint16(4, Number.parseInt(t.substring(9, 13), 16), !0), r.setUint16(6, Number.parseInt(t.substring(14, 18), 16), !0), a.set(P(t.substring(19, 23)), 8), a.set(P(t.substring(24)), 10), a;
  }
  constructor(t) {
    this.str = t;
  }
...
const Z = {
  ...
  get: (n, t) => ({
    ...
    numberOfHeaderObjects: l.get(n, t + 24)
    // Reserved: 2 bytes
  })
}, g = {
  len: 24,
...
class C {
  ...
  postProcessTag(t, a, r, s) {
    if (a === "WM/Picture")
      t.push({ id: a, value: I.fromBuffer(s) });
    else {
      const c = R(r);
      if (!c)
...
class O extends C {
  get(t, a) {
    return {
      ...
      flags: {
        broadcast: A(t, a + 64, 24),
        seekable: A(t, a + 64, 25)
      },
      // flagsNumeric: Token.UINT32_LE.get(buf, off + 64),
      minimumDataPacketSize: l.get(t, a + 68),
      maximumDataPacketSize: l.get(t, a + 72),
      maximumBitrate: l.get(t, a + 76)
    };
  }
}
...
y.guid = e.StreamPropertiesObject;
class T {
  constructor() {
    this.len = 22;
  }
...
T.guid = e.HeaderExtensionObject;
const G = {
  len: 20,
  get: (n, t) => ({
...
async function f(n) {
  const t = await n.readToken(G), a = [];
  for (let r = 0; r < t.entryCount; ++r)
    a.push(await te(n));
  return a;
}
async function ee(n) {
  const t = await n.readNumber(E), a = new Uint8Array(t);
...
class w extends C {
  get(t, a) {
    const r = [], s = new DataView(t.buffer, a);
    let c = 10;
    for (let i = 0; i < w.contentDescTags.length; ++i) {
      const d = s.getUint16(i * 2, !0);
      if (d > 0) {
        const u = w.contentDescTags[i], b = c + d;
        r.push({ id: u, value: p(t.subarray(a + c, a + b)) }), c = b;
      }
    }
    return r;
  }
...
class k extends C {
  get(t, a) {
    const r = [], s = new DataView(t.buffer, a), c = s.getUint16(0, !0);
    let i = 2;
    for (let d = 0; d < c; d += 1) {
      const u = s.getUint16(i, !0);
      i += 2;
      const b = p(t.subarray(a + i, a + i + u));
      i += u;
      const m = s.getUint16(i, !0);
      i += 2;
      const D = s.getUint16(i, !0);
      i += 2;
      const j = t.subarray(a + i, a + i + D);
      i += D, this.postProcessTag(r, b, m, j);
    }
    return r;
  }
}
k.guid = e.ExtendedContentDescriptionObject;
class x extends C {
  get(t, a) {
    const r = new DataView(t.buffer, a);
    return {
...
x.guid = e.ExtendedStreamPropertiesObject;
class F extends C {
  get(t, a) {
    const r = [], s = new DataView(t.buffer, a), c = s.getUint16(0, !0);
    let i = 2;
    for (let d = 0; d < c; d += 1) {
      ...
      i += 4;
      const D = p(t.subarray(a + i, a + i + u));
      i += u;
      const j = t.subarray(a + i, a + i + m);
      i += m, this.postProcessTag(r, D, b, j);
    }
    return r;
  }
}
...
class I {
  ...
  get(t, a) {
    const r = new DataView(t.buffer, a), s = r.getUint8(0), c = r.getInt32(1, !0);
    let i = 5;
    for (; r.getUint16(i) !== 0; )
      i += 2;
...
const B = q("music-metadata:parser:ASF"), ae = "asf";
class ne extends J {
  async parse() {
  ...
  async parseObjectHeader(t) {
    ...
    do {
      ...
      switch (B("header GUID=%s", r.objectId.str), r.objectId.str) {
        ...
        case T.guid.str: {
          const s = await this.tokenizer.readToken(new T());
        ...
        case w.guid.str:
          ...
          break;
        case k.guid.str:
          a = await this.tokenizer.readToken(new k(r)), await this.addTags(a);
          break;
        ...
        case e.PaddingObject.str:
          B("Padding: %s bytes", r.objectSize - g.len), await this.tokenizer.ignore(r.objectSize - g.len);
          break;
        default:
          this.metadata.addWarning(`Ignore ASF-Object-GUID: ${r.objectId.str}`), B("Ignore ASF-Object-GUID: %s", r.objectId.str), await this.tokenizer.readToken(new U(r));
      }
    } while (--t);
  }
  async addTags(t) {
  ...
  async parseExtensionObject(t) {
    do {
      const a = await this.tokenizer.readToken(g), r = a.objectSize - g.len;
      switch (a.objectId.str) {
        case x.guid.str:
          await this.tokenizer.readToken(new x(a));
          break;
        case F.guid.str: {
        ...
        case e.ASF_Index_Placeholder_Object.str:
          ...
          break;
        default:
          this.metadata.addWarning(`Ignore ASF-Object-GUID: ${a.objectId.str}`), await this.tokenizer.readToken(new U(a));
          break;
      }
      t -= a.objectSize;
    } while (t > 0);
