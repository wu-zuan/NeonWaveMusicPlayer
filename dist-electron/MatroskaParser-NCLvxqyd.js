import { E as w, c as f, b as S, r as h, t as E, S as T, m as I, v as $, B as z, w as D, T as N } from "./main-DilxVT5P.js";
...
class U {
  ...
  async parseContainer(a, t, l) {
    ...
    for (; this.tokenizer.position < t; ) {
      ...
      if (n)
        switch (l.startNext(n)) {
          case c.ReadNext:
            if (r.id, g(`Read element: name=${x(n)}{id=0x${r.id.toString(16)}, container=${!!n.container}} at position=${s}`), n.container) {
              const m = await this.parseContainer(n, r.len >= 0 ? this.tokenizer.position + r.len : -1, l);
              n.multiple ? (o[n.name] || (o[n.name] = []), o[n.name].push(m)) : o[n.name] = m, await l.elementValue(n, m, s);
            } else {
              const m = this.parserMap.get(n.value);
              if (typeof m == "function") {
                const d = await m(r);
                o[n.name] = d, await l.elementValue(n, d, s);
              }
            }
            break;
          case c.SkipElement:
            g(`Go to next element: name=${x(n)}, element.id=0x${r.id}, container=${!!n.container} at position=${s}`);
            break;
          case c.IgnoreElement:
            g(`Ignore element: name=${x(n)}, element.id=0x${r.id}, container=${!!n.container} at position=${s}`), await this.tokenizer.ignore(r.len);
            break;
          case c.SkipSiblings:
            g(`Ignore remaining container, at: name=${x(n)}, element.id=0x${r.id}, container=${!!n.container} at position=${s}`), await this.tokenizer.ignore(t - this.tokenizer.position);
            break;
          case c.TerminateParsing:
            return g(`Terminate parsing at element: name=${x(n)}, element.id=0x${r.id}, container=${!!n.container} at position=${s}`), o;
        }
      else
        switch (r.id) {
          case 236:
            this.padding += r.len, await this.tokenizer.ignore(r.len);
            break;
          default:
            g(`parseEbml: parent=${x(a)}, unknown child: id=${r.id.toString(16)} at position=${s}`), this.padding += r.len, await this.tokenizer.ignore(r.len);
        }
    }
    return o;
  }
...
function M(u, a) {
  const t = new Uint8Array(8), l = u.subarray(0, a);
  try {
    return t.set(l, 8 - a), $.get(t, 0);
  } catch {
    return BigInt(-1);
  }
}
  ...
  async parse() {
    ...
    p("Initializing DTD end MatroskaIterator"), await t.iterate(F, a, {
      startNext: (l) => {
        switch (l.id) {
          case 475249515:
            return p(`Skip element: name=${l.name}, id=0x${l.id.toString(16)}`), c.IgnoreElement;
          case 524531317:
            if (this.flagUseIndexToSkipClusters && this.seekHead) {
              const o = this.seekHead.seek.find((r) => r.position + this.seekHeadOffset > this.tokenizer.position);
              if (o) {
                const r = o.position + this.seekHeadOffset - this.tokenizer.position;
      ...
      elementValue: async (l, o, r) => {
        switch (p(`Received: name=${l.name}, value=${o}`), l.id) {
          case 17026:
            this.metadata.setFormat("container", `EBML/${o}`);
            break;
          case 290298740:
            this.seekHead = o, this.seekHeadOffset = r;
            break;
          case 357149030:
            {
              const s = o, n = s.timecodeScale ? s.timecodeScale : 1e6;
              if (typeof s.duration == "number") {
                const i = s.duration * n / 1e9;
                await this.addTag("segment:title", s.title), this.metadata.setFormat("duration", Number(i));
              }
            }
            break;
          case 374648427:
            {
              ...
              if (s != null && s.entries) {
                ...
                const n = s.entries.filter((i) => i.trackType === N.audio).reduce((i, m) => !i || m.flagDefault && !i.flagDefault || m.trackNumber < i.trackNumber ? m : i, null);
          ...
          case 307544935:
            {
              const s = o;
              await Promise.all(s.tag.map(async (n) => {
          ...
          case 423732329:
            {
              const s = o;
              await Promise.all(s.attachedFiles.filter((n) => n.mimeType.startsWith("image/")).map((n) => this.addTag("picture", {
                data: n.data,
                format: n.mimeType,
